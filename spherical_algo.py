...
# global function containing all iterations and stop criterion measurements
def pose_estimation(p3d_1,p3d_2,p3d_3,error_max,iteration_max):
    ...
    while abs(sv_e-sv_e_old)>error_max and count<iteration_max:
        ...
        sv_t_23=sv_t[1,3:6]
        sv_t_31=sv_t[2,6:9]
        sv_e_norm=3.0*sv_e/(np.linalg.norm(sv_t_12)+np.linalg.norm(sv_t_23)+np.linalg.norm(sv_t_31))
...
# Cometic function to build a diagonal matrix from
# rotations and translation of a given triplets
...
# SVD estimation that permit to retrive rotations
# and translations between triplet of 3D point sets
...
# a function that determine the centers of spheres on the basis of rotations and
# translations in the first referential (a document will be provided for that)
def centers_determination(sv_r,sv_t):
    ...
    c1=np.zeros(3)
    c2=-sv_t_12
    c3=-sv_t_12+np.dot(sv_r_23,-sv_t_23)
    return c1,c2,c3

# a function that determine the directions of sphere points on the basis of rotations and
# translations in the first referential (a document will be provided for that)
def azims_determination(a1,a2,a3,sv_r,sv_t):
    ...
    sv_t_23=sv_t[1,3:6]
    sv_t_31=sv_t[2,6:9]
    a1=np.dot(sv_r_12,a1)
    a3=np.dot(sv_r_23.transpose(),a3)
    return a1,a2,a3

# function for determining intersection of several lines
...
# function that permit to estimate the points distances from the center given
# estimated rotations and translations between the spheres
def estimation_rayons(p3d_1,p3d_2,p3d_3,sv_u,sv_v,sv_w,sv_r,sv_t):
    ...
    for i in range(longueur):
        ...
        sv_u.append(sv_u_ind)
        sv_v.append(sv_v_ind)
        sv_w.append(sv_w_ind)
    return sv_u,sv_v,sv_w,sv_e

# function very similar to "estimation_rayons" that permit
# to build the 3d scene after the last iteration
